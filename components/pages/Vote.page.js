...
import RemainingCredits from 'components/tools/credits'
import { Loader } from 'components/tools/loader' // Placeholder loader
import { Navigation } from 'components/tools/navigation' // Navigation component
import ProposalBlocks from 'components/tools/proposalBlocks'

function VotePage({ query }) {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        ...
        // Set name if exists
        setName(response.data.voter_name !== null ? response.data.voter_name : '')
      ...
      <div className='vote'>
        ...
        {!loading ? (
            ...
            <div className='ballot_container'>
              <div className='vote__info'>
                ...
                <div className='vote__info_heading'>
                  <h1>{t('vote.place_youe_votes')}</h1>
                  <p>
                    {t('vote.you_can_use_to')}{' '}
                    <strong>
                      {data.event_data.credits_per_voter}
                      {t('vote.credits')}
                    </strong>{' '}
                    {t('vote.during_this_event')}
                  </p>
                </div>
                  ...
                  <div className='vote__loading event__summary'>
                    ...
                    {data ? (
                      <>
                        {moment() > moment(data.event_data.end_event_date) ? (
                          <>
                            <h3>{t('vote.results')}</h3>
                            {/* Redirect to event dashboard */}
                            <Link href={`/event?id=${data.event_id}`}>
                              <a>{t('vote.see_dashboard')}</a>
                            </Link>
                          </>
                        ) : (
                          <>
                            {moment() < moment(data.event_data.start_event_date) ? (
                              <h3>
                                {t('vote.event_begin')}{' '}
                                {moment(data.event_data.start_event_date).format(
                            ...
                            {data.vote_data.map((option, i) => {
                              ...
                              return (
                                <div key={i} id={i} className='event__option_item'>
                                  <div>
                                    ...
                                    {option.description !== '' ? (
                                      ...
                                      <div id={`description-container-${i}`}>
                                        <label> {t('vote.vote_description')}</label>
                                  ...
                                  {votes[i] !== 0 ? (
                                    <ProposalBlocks cost={Math.pow(votes[i], 2)} />
                                  ) : null}
          ...
          <div className='vote__loading'>
            <h1>{t('vote.loading...')}</h1>
            <p>{t('vote.give_us_a_moment')}</p>
          </div>
        )}
      </div>
