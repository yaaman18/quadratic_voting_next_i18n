...
import { Chart } from 'react-chartjs-2' // Horizontal bar graph
...
function EventPage({ query }) {
  ...
  return (
    <div event>
      {/* Custom meta images */}
      <Head>
        <meta property='og:image' content={`https://qv-image.vercel.app/api/?id=${query.id}`} />
        <meta
      ...
      <div className='event'>
        ...
        <div className='event__information'>
          ...
          <p>{!loading && data ? data.event.event_description : 'Loading...'}</p>
          {data ? (
            <>
              {moment() > moment(data.event.end_event_date) ? (
                <h3>{t('event.this_event_has_concluded')}!</h3>
              ) : (
                <>
                  {moment() < moment(data.event.start_event_date) ? (
                    <h3>
                      {t('event.this_event_begins')}{' '}
                      {moment(data.event.start_event_date).format('MMMM Do YYYY, h:mm:ss a')}
                    </h3>
                  ) : (
                    <h3>
                      {t('event.this_event_closes')}{' '}
                      {moment(data.event.end_event_date).format('MMMM Do YYYY, h:mm:ss a')}
                    </h3>
                  )}
                </>
              )}
        ...
        {!loading && data ? (
          editMode ? (
            <div className='event__section'>
              <label>{t('event.event_start_date')}</label>
        ...
        {!loading && data ? (
          editMode ? (
            <div className='event__section'>
              <label>{t('event.event_end_date')}</label>
              <div className='event__dates'>
                <Datetime
                  className='create__settings_datetime'
                  value={endDate}
        ...
        {/* Event public URL */}
        <div className='event__section'>
          <label>{t('event.event_url')}</label>
          <p>{t('event.statistics_dashboard')}</p>
          <input value={`https://quadraticvote.radicalxchange.org/event?id=${query.id}`} readOnly />
        </div>
        ...
        data ? (
          <div className='event__section'>
            <label className='private__label'> {t('event.private_admin_url')}</label>
            <p>{t('event.save_this')}</p>
            <input
        ...
        data ? (
          <div className='event__section'>
            <label className='private__label'>{t('event.individual')}</label>
            <p>{t('event.private_sharing')}</p>
            <textarea
            ...
            <button onClick={downloadTXT} className='download__button'>
              {t('event.download_TXT')}
            </button>
          </div>
        ) : null}

        {/* Event public chart */}
        {query.id !== '' && !loading && data ? (
          <div className='event__section'>
            <label>{t('event.event_votes')}</label>
            {data.chart ? (
              <>
                <p>{t('event.Quadratic_Voting-weighted')}</p>
                {!loading && data ? (
                  <>
                    <div className='chart'>
                    ...
                    <button onClick={downloadXLSX} className='download__button'>
                      {t('event.download_apreadsheet')}
                    </button>
                  </>
                ) : (
                  <div className='loading__chart'>
                    <HashLoader size={50} color='#000' css={{ display: 'inline-block' }} />
                    <h3>{t('event.loading_chart')}</h3>
                    <span> {t('event.please_give_us_a_moment')}</span>
                  </div>
                )}
              </>
            ) : (
              <p>{t('event.voting_results')}</p>
            )}
          </div>
        ) : null}
            ...
            {data.statistics ? (
              <>
                <div className='event__sub_section'>
                  <label>{t('event.voting_participants')}</label>
                  <h3>
                    {!loading && data
                ...
                <div className='event__sub_section'>
                  <label>{t('event.credits_used')}</label>
                  <h3>
                    {!loading && data
              ...
              <p>{t('event.event_Statistics_will_appear')}</p>
            )}
          </div>
        ) : null}
      </div>
