...
export default async (req, res) => {
  ...
  const isAdmin = event.secret_key && event.secret_key === secret_key ? true : false
  ...
  if (isAdmin || moment() > moment(event.end_event_date)) {
    ...
    chart = generateChart(event.event_data, statistics.linear, statistics.qv)
  }
...
function generateChart(subjects, linearWeights, weights) {
  ...
  sorted_data = sorted_data.sort((a, b) => {
    if (b.data - a.data !== 0) {
      ...
      return b.linearData - a.linearData
    }
  })
